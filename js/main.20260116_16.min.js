(function(){
'use strict';var nav = document.getElementById('main-nav');
if(!nav) return;// Desktop-like hover detection
var canHover = false;
try {
canHover = window.matchMedia && window.matchMedia('(hover: hover) and (pointer: fine)').matches;
} catch(e) {}function qsa(sel, root){
...
var links = qsa('#main-nav a, #main-nav .submenu a', document);links.forEach(function(a){
try {
...
if(sub){ closeAll(); return; }// Only intercept clicks on top labels when they are NOT real links.
var top = e.target && e.target.closest ? e.target.closest('#main-nav li.has-sub > a') : null;
if(!top) return;var href = (top.getAttribute('href') || '').trim();
var isHash = (href === '' || href === '#' || href.toLowerCase().indexOf('javascript:') === 0);if(isHash){
e.preventDefault();
var li = top.closest('#main-nav li.has-sub');
if(!li) return;var willOpen = !li.classList.contains('is-open');
closeAll(li);
if(willOpen){
...
if(!li) return;var href = (a.getAttribute('href') || '').trim();
var isHash = (href === '' || href === '#' || href.toLowerCase().indexOf('javascript:') === 0);// If no real href, always toggle only
if(isHash){
e.preventDefault();
...
if(!canHover) return;var CLOSE_DELAY_MS = 150;
qsa('#main-nav li.has-sub', nav).forEach(function(li){
var t = null;li.addEventListener('mouseenter', function(){
...
});// 자동 가나다 정렬 적용function sortByKoreanName(arr){
